...
public class ComparisonProcess : IDataProcess
{
    ...
    ///     单段比对统计
    /// </summary>
    protected class SegStatisticsForComparison(double startFreq, double stopFreq, double stepFreq)
    {
        ...
        {
            var weakSignal = new List<Tuple<int, float, float>>();
            for (var i = 0; i < _weakSignal.Length; i++)
            {
                ...
                {
                    var ave = _totalLevel[i] / _dataTimes;
                    weakSignal.Add(new Tuple<int, float, float>(i, _maxLevel[i], ave));
                }
            }
        ...
        private readonly double _stopFreq = stopFreq;

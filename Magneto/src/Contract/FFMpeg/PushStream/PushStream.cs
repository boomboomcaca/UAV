...
///     内存读取
/// </summary>
internal unsafe class PushStream
{
    /// <summary>
    ///     AVIO 内存大小
    /// </summary>
        ...
        bool enableLog = false)
    {
        ...
        void LogCallback(void* p0, int level, string format, byte* vl)
        {
            ...
            if (enableLog) Debug.WriteLine(line);
        }
        ...
        while (!_isStop)
        {
            ...
            */
            ...
            if (pkt.stream_index == videoindex)
            {
                ...
                if (ptsTime > nowTime)
                {
                    var sleep = (uint)(ptsTime - nowTime);
            ...
            dts = pkt.dts;
            // 计算延时后，重新指定时间戳
            // TODO : 测试这个枚举"AVRounding"是否有问题-暂时没问题
            ...
            // 打印视频数据发送信息
            if (pkt.stream_index == videoindex)
            // Console.WriteLine($"Send {frame_index} video frames to output URL\r\n");
            // Console.WriteLine($"推送 {frame_index} 帧数据.....", "视频数据推送日志");
            {
            }
            ...
            case IoFillModel.ContinueWrite:
                {
                    ...
                    if (MDataCacheManager.Instance != null)
                    {
                        ...
                        if (mdata != null)
                        {
                            ...
                            if (msize)
                                // Console.WriteLine($"[{mdata.Size}<{bufsize}]缓存数据不足，等待中...", "缓存数据大小检查日志");
                                // 缓存数据不足，则循环等待
                                while (msize)
                                {
                            ...
                            // Console.WriteLine("缓存数据已填充，可以读取数据.", "缓存数据大小检查日志");
                            // 获取数据块

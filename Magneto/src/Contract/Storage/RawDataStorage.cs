...
/// </summary>
public sealed class RawDataStorage
{
    ...
    private const long SplitSize = 2000 * 1024 * 1024;
    private static readonly Lazy<RawDataStorage> _lazy = new(() => new RawDataStorage());
    private readonly string _rootPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, PublicDefine.PathSavedata);
    public readonly ConcurrentDictionary<Guid, TaskFileManagerInfo> TaskFileManagerInfoList = new();
    ...
    /// <summary>
    ///     本地存储格式：3-WAV,4-MP3,5-WMA
    /// </summary>
    private int _format;

    private RawDataStorage()
    {
    }

    // private bool _disposed = false;
    public static RawDataStorage Instance => _lazy.Value;
        ...
        List<Parameter> parameters)
    {
        ...
        if (Directory.Exists(taskPath)) Directory.Delete(taskPath, true);
        Directory.CreateDirectory(taskPath);
        //生成文件名
        var featureName = Utils.ConvertEnumToString(feature);
        const ushort fileIndex = 1;
        var name = $"{header.EdgeId}_{featureName}_{createdTime:yyyyMMddHHmmss}";
        ...
        TaskFileManagerInfoList.TryAdd(info.TaskId, info);
        //设置音频保存格式
        var rawParam = parameters.Find(item => item.Name == ParameterNames.RawSwitch);
        if (rawParam != null && int.TryParse(rawParam.Value.ToString(), out var saveSign))
            if (saveSign > 2)
            {
                Trace.WriteLine($"当前音频存储格式为:{new[] { "WAV", "MP3", "WMA" }[saveSign - 3]}");
                _format = saveSign;
            }
    }
    ...
    public void ChangeParameters(Guid taskId, List<Parameter> parameters, bool newCycle = false)
    {
        ...
        if (!TaskFileManagerInfoList.TryGetValue(taskId, out var managerInfo) || managerInfo == null) return;
        ...
        lock (managerInfo.DataSyncLock)
        {
            ...
            //设置音频保存格式
            var rawParam = parameters.FirstOrDefault(item => item.Name == ParameterNames.RawSwitch);
            if (rawParam != null && int.TryParse(rawParam.Value.ToString(), out var saveSign))
                if (saveSign > 2)
                {
                    Trace.WriteLine($"当前音频存储格式为:{new[] { "WAV", "MP3", "WMA" }[saveSign - 3]}");
    ...
    public bool IsTaskWriting(Guid taskId)
    {
        if (!TaskFileManagerInfoList.TryGetValue(taskId, out var managerInfo) || managerInfo == null) return false;
        return managerInfo.IsRunning;
    }
    ...
    public void Write(Guid taskId, List<object> data)
    {
        ...
        lock (managerInfo.DataSyncLock)
        {
            ...
            if (data.Find(item => item is SDataAudio) is SDataAudio audio && _format >= 3)
            {
                var fileName = managerInfo.FileNotificationInfo.FileName;
                var rootPath = managerInfo.FileNotificationInfo.RootPath;
                var folder = managerInfo.FileNotificationInfo.RelativePath;
                SaveAudio(audio, rootPath, folder, fileName);
            }
        }
    }
    ...
    public void Complete(Guid taskId)
    {
        ...
        TaskFileManagerInfoList.Remove(managerInfo.TaskId, out _);
    }
    ...
    {
        if (!TaskFileManagerInfoList.TryGetValue(taskId, out var managerInfo) || managerInfo == null) return 0;
    ...
    /// <returns>文件完整名称包括路径</returns>
    public string GetFileName(Guid taskId)
    {
        if (!TaskFileManagerInfoList.TryGetValue(taskId, out var managerInfo) || managerInfo == null)
            return string.Empty;
        if (!managerInfo.IsRunning) return string.Empty;
        return managerInfo.TempIndexFileName;
    }
    ...
    public static void ReadFile(string folderPath, string fileName, out FileDataSummary summary,
    ...
    public object[][] GetFrame(string folderPath, string fileName, int frameCount)
    {
        folderPath = _rootPath + "/" + folderPath;
        var idxFile = Path.Combine(folderPath, fileName + IndexSuffix);
        var paramsFile = Path.Combine(folderPath, fileName + ParamSuffix);
        if (!File.Exists(idxFile) || !File.Exists(paramsFile)) return null;
        var dataFiles = Directory.GetFiles(folderPath, "*" + DataSuffix);
        if (dataFiles.Any() != true) return null;
        //读参数文件
        using var paramsStream = File.OpenRead(paramsFile);
        MessagePackSerializer.Deserialize<ParametersInfo>(paramsStream);
        //读索引文件
        uint index = 0;
        using var idxStream = File.OpenRead(idxFile);
        idxStream.Seek(index, SeekOrigin.Begin);
        var frameTotalArray = new byte[4];
        _ = idxStream.Read(frameTotalArray, 0, frameTotalArray.Length);
        BitConverter.ToUInt32(frameTotalArray);
        index += (uint)frameTotalArray.Length;
        idxStream.Seek(index, SeekOrigin.Begin);
        var headerLenArray = new byte[4];
        _ = idxStream.Read(headerLenArray, 0, headerLenArray.Length);
        ...
        var headerArray = new byte[headerLen];
        _ = idxStream.Read(headerArray, 0, headerArray.Length);
        MessagePackSerializer.Deserialize<FileDataSummary>(headerArray);
        index += headerLen;
        var frameList = new List<(long dataIndex, ushort fileIndex, int position, ulong timestamp)>();
        index += (uint)(IndexSize * frameCount);
        idxStream.Seek(index, SeekOrigin.Begin);
        var tempIdxArray = new byte[IndexSize];
        _ = idxStream.Read(tempIdxArray, 0, tempIdxArray.Length);
        GetIndexValue(tempIdxArray, out var dataIndex, out var fileIndexBuf, out var position, out var timestamp);
        frameList.Add((dataIndex, fileIndexBuf, position, timestamp));
        //读取数据文件
        var pattern = $"^{fileName}\\.(?<fileIndex>\\d+)\\{DataSuffix}$";
        var list = new List<(long dataIndex, object data)>();
        foreach (var file in dataFiles)
        ...
        var data = list.OrderBy(p => p.dataIndex).Select(p => p.data).ToArray();
        if (data.Length < 1) return null;
        var reData = (object[])((object[])data.First())[0];
        var re = new object[reData.Length][];
        for (var i = 0; i < reData.Length; i++) re[i] = (object[])((Dictionary<object, object>)reData[i])["data"];
        return re;
    }

    private static void SaveNewDataFile(TaskFileManagerInfo managerInfo, bool newCycle = false)
    {
        //保存旧文件
        RecoveryFileProcessed(managerInfo.TempDataFileName);
        //更新参数信息
    ...
    private static string GetMainParaString(List<Parameter> parameters)
    {
    ...
    private static byte[] CreateIndexBytes(long dataIndex, ushort fileIndex, int position, ulong timestamp)
    {
    ...
    private static void GetIndexValue(byte[] array, out long dataIndex, out ushort fileIndex,
    ...
    /// </summary>
    /// <param name="fileName"></param>
    private static string RecoveryFileProcessed(string fileName)
    {
        if (string.IsNullOrEmpty(fileName)) return null;
    ...
    private void SaveAudio(SDataAudio audio, string rootPath, string folder, string fileName)
    {
        if (_audioSave == null)
        {
            _audioSave = new AudioDataSave();
            _audioSave.SaveStart(rootPath, folder, fileName, 0, audio.Channels, audio.SamplingRate,
                audio.BitsPerSample);
        }
    ...
    private void StopSaveAudio()
    {
        ...
        _audioSave.SaveComplete(_format);
        _audioSave.Dispose();
        _audioSave = null;
    }

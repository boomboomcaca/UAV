...
public partial class ParallelEthAntController : AntennaControllerBase
{
    #region 构造函数

    public ParallelEthAntController(Guid id) : base(id)
    {
    }
    ...
    {
        //初始化与天线控制器1连接
        var ep1 = new IPEndPoint(IPAddress.Parse(AntController1Ip), Port1);
        _controllerSocket1 = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        _controllerSocket1.NoDelay = true;
        _controllerSocket1.Connect(ep1);
        var ep2 = new IPEndPoint(IPAddress.Parse(AntController2Ip), Port2);
        _controllerSocket2 = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        _controllerSocket2.NoDelay = true;
        _controllerSocket2.Connect(ep2);
    }
    ...
    private bool SendControllerCode2(string code)
    {
        if (_controllerSocket2?.Connected == true)
            try
            {
                ...
                return SendAntCode(_writer2, code);
            }
            catch
            {
            }

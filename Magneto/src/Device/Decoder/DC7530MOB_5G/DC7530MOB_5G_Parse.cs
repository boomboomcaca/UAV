...
public partial class Dc7530Mob5G
{
    ...
    {
        Console.WriteLine($"Ch1解析数据:{s}");
        var list = new List<SDataCellular>();
        if (commandType != CommandType.Nr) return list;
        var pattern = @"(\^)?NETSCAN\s*\:\s*((?<value>[0-9a-zA-Z\-\._]*)\s*\,\s*){15}(?<value>[0-9a-zA-Z\-\._]+)\s*";
        ...
        foreach (Match match in matches)
        {
            ...
            var s5Gscs = valueCaps[12].Value;
            var s5Grsrp = valueCaps[13].Value;
            var s5Grsrq = valueCaps[14].Value;
            var s5Grssnr = valueCaps[15].Value;
            var success = ChanNumToFreq(DuplexMode.Nr5G, arfcn, out var freq);
            if (!success) continue;
            var rxPower = ToDouble(s5Grsrp);
            var data = new SDataCellular
            {
                ...
                Mnc = ToUInt32(mnc),
                Mcc = ToUInt32(mcc),
                Lac = ToUInt32(tac, true),
                Ci = ToUInt64(cid, true),
                Bandwidth = ToInt32(band) / 1e3,
                ...
                {
                    { "arfcn", new ExtendedInfo("频点", arfcn) },
                    { "tac", new ExtendedInfo("位置区编号", tac) },
                    { "mnc", new ExtendedInfo("系统编号", mnc) },
                    { "mcc", new ExtendedInfo("国家编号", mcc) },
                    { "bsic", new ExtendedInfo("GSM小区基站码", bsic) },
                    { "rxlvel", new ExtendedInfo("接收电平", rxlvel) },
                    { "cid", new ExtendedInfo("小区编号", cid) },
                    { "band", new ExtendedInfo("带宽", band) },
                    { "psc", new ExtendedInfo("主扰码", psc) },
                    { "pci", new ExtendedInfo("物理小区号", pci) },
                    { "5GSCS", new ExtendedInfo("NR副载波间隔", s5Gscs) },
                    { "5GRSRP", new ExtendedInfo("NR参考信号接收功率", s5Grsrp) },
                    { "5GRSRQ", new ExtendedInfo("NR参考信号接收质量", s5Grsrq) },
                    { "5GRSSNR", new ExtendedInfo("对应NR物理层的SINR", s5Grssnr) }
                }
            };
            list.Add(data);
        }
    ...
    private List<SDataCellular> ParseCh2Data(string s, CommandType commandType)
    {
        ...
        var list = new List<SDataCellular>();
        if (commandType != CommandType.Gsm && commandType != CommandType.Wcdma && commandType != CommandType.TdScdma &&
        ...
        foreach (Match match in matches)
        {
            ...
            switch (commandType)
            {
                case CommandType.Gsm:
                    data = GetGsm(valueCaps);
                    break;
                case CommandType.Wcdma:
    ...
    private SDataCellular ParseCdmaData(string s, CommandType commandType)
    {
        ...
        var pattern =
            @"(CH3\s*\:)?\s*(\^)?SIQ\s*\:\s*((?<value>[0-9a-zA-Z\-\._%]*)\s*\,\s*){14}(?<value>[0-9a-zA-Z\-\._%]+)\s*";
    ...
    private void AddGpsToCdma(string s, ref SDataCellular dataCdma)
    {
        ...
        var pattern = @"(CH3\s*\:)?\s*(\^)?BSIN\s*\:\s*((?<value>\w*)\s*\,\s*){4}(?<value>\w+)";
        var match = Regex.Match(s, pattern);
    ...
    {
        var pattern = @"(CH3\s*\:)?\s*(\^)?CURRSID\s*\:\s*((?<value>\w*)\s*\,\s*){2}(?<value>\w+)";
        var match = Regex.Match(s, pattern);
        if (!match.Success) return;
        var valueCaps = match.Groups["value"].Captures;
        if (valueCaps.Any() != true) return;

...
public struct Avidm
{
    ...
    public override string ToString()
    {
        ...
        sbuilder.Append(PacketType).Append(",").Append(SegCount).Append(",").Append(Fragment).Append(",")
            .Append(Identify).Append(",").Append(Channel).Append(",").Append(Body).Append(",").Append(End);
        return sbuilder.ToString();
    }

    public override bool Equals(object obj)
    {
        if (obj is not Avidm avidm)
            return false;
        if (PacketType == avidm.PacketType &&

...
public partial class Dc7530Mob
{
    ...
    {
        while (_dataProcTokenSource?.IsCancellationRequested == false)
            try
            {
                if (!_msgBuffer.TryDequeue(out var str))
                {
                    Thread.Sleep(100);
                    continue;
                }
    ...
    private void ExecuteSendCmd()
    {
        var commandInfos = new List<DuplexCommandInfo>
        {
            ...
            new()
            {
                ...
                Expression = p => p % 16 == 2,
                Enable = Wcdma && UseChannel1,
                Commands = new List<(string cmd, CommandType commandType)>
                {
                    ("CH1:AT^NETSCAN=4,-110,1\r\n", CommandType.Ch1Wcdma)
                }
            },
            new()
            {
            ...
            new()
            {
                Channel = 2,
                NetworkStandard = NetworkStandard.Wcdma,
                Expression = p => p % 16 == 10,
                Enable = Wcdma && UseChannel2,
                Commands = new List<(string cmd, CommandType commandType)>
                {
                    ("CH2:AT^NETSCAN=4,-110,1\r\n", CommandType.Ch2Wcdma)
                }
            },
            new()
            {

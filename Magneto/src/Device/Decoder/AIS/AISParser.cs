...
public class AisParser
{
    ...
    {
        if (!IsValidAis(encodedMsg)) throw new Exception("不是合法的!AVIDM\\!AVIDO包数据，或者包含不合法字符!!");
    ...
    public static bool IsValidAis(string ais)
    {
        ...
        if (!ValidCrc(msg)) return false;
        var isValid = _pattern.IsMatch(msg);
        return _patternVdo.IsMatch(msg) || isValid;
    }

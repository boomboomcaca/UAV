...
public partial class RFeye
{
    ...
    {
        //先订阅频谱数据，否则电平数据会先返回，而在多任务运行时单频测量功能只要有一次电平数据返回就
        //认为可以切换到其它任务，会导致单频测量看不到频谱数据
        if ((_media & MediaType.Spectrum) > 0)
        {
            //通过扫描获取频谱数据
            var fstartFreq = Frequency - IfBandwidth / 1000 / 2;
            var fstopFreq = Frequency + IfBandwidth / 1000 / 2;
            var nStartFreqMHz = (int)fstartFreq;
            var nStartFreqMilliHz = (int)((fstartFreq - nStartFreqMHz) * 1000000000);
            var nStopFreqMHz = (int)fstopFreq;
            var nStopFreqMilliHz = (int)((fstopFreq - nStopFreqMHz) * 1000000000);
            ResBandWidthHz = GetRbwHz(CurFeature, IfBandwidth);
            ...
            requestPacket.AddParamInt(PacketKey.SweepAntennaUid, _antennaCode);
        ...
        var nDecimation = 20000 / (int)FilterBandwidth;
        GetFreqMHzAndOffsetHz(Frequency, out var nCenterFreqMHz, out var nDdsOffsetHz);
        requestPacket.AddField(PacketKey.FieldTime, -1);
        requestPacket.AddParamInt(PacketKey.TimeCenterFreqMHz, nCenterFreqMHz);
        requestPacket.AddParamInt(PacketKey.TimeDdsOffsetHz, nDdsOffsetHz);
        //计算ITU数据至少需要8192个点
        //TODO:点数越多计算出来的电平越平稳，此处暂统一采集8192个点
        ...
        requestPacket.AddParamInt(PacketKey.TimeDdsScale, 0);
        requestPacket.AddParamInt(PacketKey.TimeDecimation, nDecimation);
        if (Attenuation == -1)
            requestPacket.AddParamInt(PacketKey.TimeAgcTableMode, 1);
        else
            requestPacket.AddParamInt(PacketKey.TimeManualAtten, Attenuation);
        requestPacket.AddParamInt(PacketKey.TimeAntennaUid, _antennaCode);
        if ((_media & MediaType.Audio) <= 0) return;
        var nRadioTuneFreqMHz = (int)Frequency;
        var nDdsFreqKHz = (int)((Frequency - nRadioTuneFreqMHz) * 1000); //TODO:
        requestPacket.AddField(PacketKey.FieldDemodulation, -1);
        requestPacket.AddParamInt(PacketKey.DemodRadioTuneFreqMHz, nRadioTuneFreqMHz);
        requestPacket.AddParamInt(PacketKey.DemodDdsFreqKHz, nDdsFreqKHz);
        requestPacket.AddParamInt(PacketKey.DemodRfDecimation, nDecimation);
        if (DemMode == Modulation.Fm)
            requestPacket.AddParamInt(PacketKey.DemodModeFm, 1);
        else if (DemMode == Modulation.Am) requestPacket.AddParamInt(PacketKey.DemodModeAm, 1);
        requestPacket.AddParamInt(PacketKey.AnyAntennaUid, _antennaCode);
    }
    ...
    {
        var nStartFreqMHz = (int)StartFrequency;
        var nStartFreqMilliHz = (int)((StartFrequency - nStartFreqMHz) * 1000000000);
        var nStopFreqMHz = (int)StopFrequency;
        var nStopFreqMilliHz = (int)((StopFrequency - nStopFreqMHz) * 1000000000);
    ...
    private void InitTdoaRequestPacket(Packet requestPacket)
    {
        GetFreqMHzAndOffsetHz(Frequency, out var nCenterFreqMHz, out var nDdsOffsetHz);
        var nDecimation = 20000 / (int)FilterBandwidth;
        //TODO: 目前TOOA中暂时电平数据和频谱数据都通过IQ数据计算得到
        requestPacket.AddField(PacketKey.FieldTime, -1);
        requestPacket.AddParamInt(PacketKey.TimeCenterFreqMHz, nCenterFreqMHz);
        requestPacket.AddParamInt(PacketKey.TimeDdsOffsetHz, nDdsOffsetHz);
        requestPacket.AddParamInt(PacketKey.TimeNumSamples, TimeNumSamples);
        ...
        {
            //依赖于系统时间以绝对时间间隔触发
            requestPacket.AddParamInt(PacketKey.TimeTrigModeAbsTime, 1);
            requestPacket.AddParamInt(PacketKey.TimeTrigAbsUnix, 0);
            requestPacket.AddParamInt(PacketKey.TimeTrigAbsNano, 0);
            requestPacket.AddParamInt(PacketKey.TimeTrigRepeatUnix, 1);
            requestPacket.AddParamInt(PacketKey.TimeTrigRepeatNano, 0); //TODO
        }
    }
    ...
    private void InitFreqScanRequestPacket(Packet requestPacket)
    {
        ...
        var demodulation = _scanFreqs[_scanFreqIndex].Demodulation;
        var resBw = GetResBw(CurFeature, ifBw);
        if ((_media & MediaType.Spectrum) > 0)
        {
            //通过扫描获取频谱数据
            var fstartFreq = frequency - ifBw / 1000 / 2;
            var fstopFreq = frequency + ifBw / 1000 / 2;
            var nStartFreqMHz = (int)fstartFreq;
            var nStartFreqMilliHz = (int)((fstartFreq - nStartFreqMHz) * 1000000000);
            var nStopFreqMHz = (int)fstopFreq;
            var nStopFreqMilliHz = (int)((fstopFreq - nStopFreqMHz) * 1000000000);
            ...
            requestPacket.AddParamInt(PacketKey.SweepStopFreqMHz, nStopFreqMHz);
            requestPacket.AddParamInt(PacketKey.SweepStopFreqMilliHz, nStopFreqMilliHz);
            requestPacket.AddParamInt(PacketKey.SweepResBandwidthHz, resBw);
            requestPacket.AddParamInt(PacketKey.SweepAntennaUid, _antennaCode);
            requestPacket.AddParamInt(PacketKey.SweepRefLevel, SweepRefLevel);
            requestPacket.AddParamInt(PacketKey.SweepNumLoops, 1);
            if (Attenuation != -1) requestPacket.AddParamInt(PacketKey.SweepManualAtten, Attenuation);
            requestPacket.AddParamInt(
                Detector == DetectMode.Pos ? PacketKey.SweepGetPeakData : PacketKey.SweepGetAverageData, 1);
        }
        ...
        //
        var nDecimation = 20000 / (int)ifBw;
        GetFreqMHzAndOffsetHz(frequency, out var nCenterFreqMHz, out var nDdsOffsetHz);
        requestPacket.AddField(PacketKey.FieldTime, -1);
        requestPacket.AddParamInt(PacketKey.TimeCenterFreqMHz, nCenterFreqMHz);
        requestPacket.AddParamInt(PacketKey.TimeDdsOffsetHz, nDdsOffsetHz);
        const int timeNumSamples = 2048; //TODO:
        requestPacket.AddParamInt(PacketKey.TimeNumSamples, timeNumSamples);
        requestPacket.AddParamInt(PacketKey.TimeDdsScale, 0);
        requestPacket.AddParamInt(PacketKey.TimeDecimation, nDecimation);
        if (Attenuation == -1)
        ...
        requestPacket.AddParamInt(PacketKey.DemodDdsFreqKHz, nDdsFreqKHz);
        requestPacket.AddParamInt(PacketKey.DemodRfDecimation, nDecimation);
        if (demodulation == Modulation.Fm)
            requestPacket.AddParamInt(PacketKey.DemodModeFm, 1);
        else if (demodulation == Modulation.Am) requestPacket.AddParamInt(PacketKey.DemodModeAm, 1);
    }
    ...
    private void InitScanFreqs()
    {
        ...
        if (CurFeature == FeatureType.FScne)
        {
            ...
            {
                _scanFreqs.Add(new ScanFreqInfo(freq, FilterBandwidth, DemMode));
                freq = decimal.ToDouble(new decimal(StartFrequency + ++i * StepFrequency / 1000d));
            }
        }
    ...
    private void NextFrequency(float level)
    {
        ...
        {
            var bReceivedSpec = (_media & MediaType.Spectrum) <= 0 || _bReceivedSpec;
            var bReceivedAudio = (_media & MediaType.Audio) <= 0 || _bReceivedAudio;
            ...
            //result = _bReceivedScan && bReceivedSpec && bReceivedAudio 
            //    && (_watcherHoldTime.ElapsedMilliseconds > (_holdTime * 1000f) || _watcherDwellTime.ElapsedMilliseconds > (_dwellTime * 1000f));
            result = _watcherHoldTime.ElapsedMilliseconds >= HoldTime * 1000f ||
                     (_watcherDwellTime.ElapsedMilliseconds >= DwellTime * 1000f && _bReceivedScan
                         && bReceivedSpec && bReceivedAudio);
        }
    ...
    private void ResetReceivedFlags()
    {
        ...
        _bReceivedSpec = false;
        _bReceivedAudio = false;
        _watcherHoldTime.Reset();

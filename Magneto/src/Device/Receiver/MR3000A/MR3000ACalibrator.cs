...
internal enum ChannelCalibrationOperation
{
    ReadHit,
    ReadMiss,
    WritenHit,
...
internal class ChannelCalibrationProcessEventArgs : EventArgs
{
    public ChannelCalibrationProcessEventArgs(ChannelCalibrationOperation operation, double frequency, double bandwidth)
    {
        Operation = operation;
        Frequency = frequency;
        Bandwidth = bandwidth;
    ...
    public ChannelCalibrationOperation Operation { get; private set; }
...
internal interface IChannelCalibration
{
    ...
    void WriteData(double frequency, double bandwidth, float[] calibrations);
    event EventHandler<ChannelCalibrationProcessEventArgs> ChannelCalibrationProcess;
}
...
internal class MonoDualDFindCalibrator : IDFindCalibration
{
    #region 构造函数

    private MonoDualDFindCalibrator()
    {
    }
    ...
    public static MonoDualDFindCalibrator CreateInstance(DfAntennaInfo antennaInfo, bool isMonoDFind = false,
        string foldername = "", bool createNewFile = false)
    {
        var calibrator = new MonoDualDFindCalibrator
        {
        ...
        calibrator._isMonoDFind = isMonoDFind;
        try
        {
        ...
        if (tmp > previous && tmp < next)
        {
            ...
            if (_isMonoDFind)
                for (var index = 0; index < length; ++index)
                    ...
                    data[index] = GetLevelByRatio(lowData[index], highData[index], coe);
            else
                for (var index = 0; index < length; ++index)
                    // data[index] = coe * (highData[index] - lowData[index]) + lowData[index];
    ...
    private static float GetLevelByRatio(float beginLevel, float endLevel, float ratio)
    {
        if (ratio is <= 0 or > 1) throw new InvalidDataException("ratio should be larger than 0 and less than 1");
        var data = ratio * (endLevel - beginLevel) + beginLevel;
        return data;
    }
...
internal class NineChannelCalibrator : IChannelCalibration
{
        ...
        bool createNew = false, int startFreqeuncy = 20, int stopFrequency = 3600)
    {
        ...
        var gears = validIfBandwidths?.Length ?? 1;
        calibrator._buffer =
    ...
    public event EventHandler<ChannelCalibrationProcessEventArgs> ChannelCalibrationProcess;
    ...
    public float[] ReadData(double frequency, double bandwidth)
    {
        ...
        var gears = _sortedValidIfBandwidths?.Length ?? 1;
        ...
        ChannelCalibrationProcess?.Invoke(this, null);
        return buffer;
    }

    public void WriteData(double frequency, double bandwidth, float[] calibrations)
    {
    }
...
internal class RealtimeNineChannelCalibrator : IChannelCalibration
{
    private readonly object _calibratingLock = new();

    private readonly IDictionary<int, IDictionary<int, float[]>> _frequencyBandwidthCalibrationDic =
        new Dictionary<int, IDictionary<int, float[]>>();

    private readonly int[] _sortedValidIfBandwidths;

    private bool _calibrating;

    public RealtimeNineChannelCalibrator(int[] validIfBandwidths = null)
    {
        if (validIfBandwidths != null) Array.Sort(validIfBandwidths, (x, y) => x.CompareTo(y));
        _sortedValidIfBandwidths = validIfBandwidths;
    }

    #region IChannelCalibration

    public bool Calibrating
    {
        get
        {
            lock (_calibratingLock)
            {
                return _calibrating;
            }
        }
    ...
    public float[] ReadData(double frequency, double bandwidth)
    {
        var tempFrequency = (int)frequency;
        var bandwidthIndex = GetGearIndex(bandwidth);
        if (!_frequencyBandwidthCalibrationDic.ContainsKey(tempFrequency) ||
            !_frequencyBandwidthCalibrationDic[tempFrequency].ContainsKey(bandwidthIndex))
        {
            ProcessChannelCalibration(frequency, bandwidth, ChannelCalibrationOperation.ReadMiss);
            return null;
        }

        ProcessChannelCalibration(frequency, bandwidth, ChannelCalibrationOperation.ReadHit);
        var tempBuffer = _frequencyBandwidthCalibrationDic[tempFrequency][bandwidthIndex];
        var buffer = new float[tempBuffer.Length];
        Buffer.BlockCopy(tempBuffer, 0, buffer, 0, sizeof(float) * buffer.Length);
        return buffer;
    }
    ...
    public void WriteData(double frequency, double bandwidth, float[] calibrations)
    {
        ...
        var bandwidthIndex = GetGearIndex(bandwidth);
        if (!_frequencyBandwidthCalibrationDic.ContainsKey(tempFrequency))
            _frequencyBandwidthCalibrationDic[tempFrequency] = new Dictionary<int, float[]>();
        _frequencyBandwidthCalibrationDic[tempFrequency][bandwidthIndex] = calibrations;
        ProcessChannelCalibration(frequency, bandwidth, ChannelCalibrationOperation.WritenHit);
    }
    ...
    private void ProcessChannelCalibration(double frequency, double bandwidth, ChannelCalibrationOperation operation)
    {
        var args = new ChannelCalibrationProcessEventArgs(operation, frequency, bandwidth);
        ChannelCalibrationProcess?.Invoke(this, args);
        switch (operation)
        {
            ...
            case ChannelCalibrationOperation.WritenMiss:
                Calibrating = true;
                break;
            case ChannelCalibrationOperation.ReadHit:

...
internal class ExtractionInfo
{
    ...
    public ExtractionInfo(byte[] value, ref int startIndex)
    {
        ...
        if (FhType.Value == 0)
        {
            List<FhChannelInfo> tempChannels = new();
            for (var i = 0; i < NumOfSubbandsOrChannels.Value; ++i)
            {
                FhChannelInfo tempChannel = new(value, ref startIndex);
        ...
        {
            List<FhSubBandInfo> tempSubBands = new();
            for (var i = 0; i < NumOfSubbandsOrChannels.Value; ++i)
            {
                FhSubBandInfo tempSubband = new(value, ref startIndex);

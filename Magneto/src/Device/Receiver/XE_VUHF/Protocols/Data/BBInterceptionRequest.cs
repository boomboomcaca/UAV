...
internal class BbInterceptionRequest
{
    ...
    #region 用于进行PhaseNo的赋值

    //每次发送该消息时自增1进行赋值以示区别
    private static volatile uint _phaseNoValue;
    ...
    public byte[] GetBytes(uint version)
    {
        ...
        Interlocked.Increment(ref _phaseNoValue);
        PhaseNo.Value = _phaseNoValue;

        #endregion

        Header.ContentSize = GetSize() - MessageHeader.GetSize();
        if (version != 25) Header.ContentSize -= ShortField.GetSize();
        List<byte> bytes = new();

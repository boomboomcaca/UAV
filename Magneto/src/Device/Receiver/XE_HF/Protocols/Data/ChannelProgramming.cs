...
internal class ChannelProgramming
{
    ...
    public int GetSize()
    {
        return Channel.GetSize() + ChannelNo.GetSize() + FMin.GetSize() + FMax.GetSize() +
               RfAttenuator.GetSize() + IfAttenuator.GetSize() + AmpliConfig.GetSize() + Antenna.GetSize() +
               FmFilter.GetSize() + LevelUnits.GetSize() + AgcType.GetSize();
    }
}
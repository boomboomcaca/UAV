...
internal class RawData
{
    ...
    public ushort DdcCount { get; private set; }
    public ushort DdcIndex { get; private set; }
    ...
    public static RawData Parse(byte[] value, int offset, int packetVersion = 0)
    {
        ...
        switch (tag)
        {
            case DataType.Iq:
            case DataType.Tdoa:
                if (packetVersion >= 2) raw = new RawIq();
                break;
            case DataType.Level:
                raw = new RawLevel();
                break;
            ...
            case DataType.Itu:
                raw = new RawItu();
                break;
            case DataType.Scan: // FSCAN
            case DataType.Scan + 2: // MSCAN
            case DataType.Scan + 4: // PScan
            {
                var version = BitConverter.ToUInt32(value, offset + 4);
                if (version == 40)
                    raw = new RawScan();
    ...
    public virtual int Convert(byte[] value, int offset)
    {
        ...
        offset += 4;
        DdcCount = BitConverter.ToUInt16(value, offset);
        offset += 2;
        DdcIndex = BitConverter.ToUInt16(value, offset);
        offset += 2;
...
[Serializable]
internal class RawDfScan : RawData
{
    public int SegmentIndex { get; set; }
    public long StartFrequency { get; set; }
    public long StopFrequency { get; set; }
    public long StepFrequency { get; set; }
    public int TotalCount { get; set; }
    public int Offset { get; set; }
    public int DataCount { get; set; }
    public int SignalCount { get; set; }
    public short[] LevelArray { get; set; }
    public short[] AzimuthArray { get; set; }
    public short[] QualityArray { get; set; }
    public int[] IndexPairs { get; set; }
    ...
    public override int Convert(byte[] value, int offset)
    {
        ...
        StepFrequency = BitConverter.ToInt64(value, offset);
        offset += 8;
        TotalCount = BitConverter.ToInt32(value, offset);
        offset += 4;
        Offset = BitConverter.ToInt32(value, offset);
        offset += 4;
        DataCount = BitConverter.ToInt32(value, offset);
        offset += 4;
        SignalCount = BitConverter.ToInt32(value, offset);
        offset += 4;

        LevelArray = new short[DataCount];
        Buffer.BlockCopy(value, offset, LevelArray, 0, DataCount * 2);
        offset += DataCount * 2;
        AzimuthArray = new short[DataCount];
        Buffer.BlockCopy(value, offset, AzimuthArray, 0, DataCount * 2);
        offset += DataCount * 2;
        QualityArray = new short[DataCount];
        Buffer.BlockCopy(value, offset, QualityArray, 0, DataCount * 2);
        offset += DataCount * 2;
        IndexPairs = new int[SignalCount * 2];
        Buffer.BlockCopy(value, offset, IndexPairs, 0, SignalCount * 2 * 4);
        offset += SignalCount * 2 * 4;
        return offset;
    }
    ...
    {
        return
            $"{base.ToString()}, seg_idx={SegmentIndex}, start_freq={StartFrequency}, stop_freq={StopFrequency}, step_freq={StepFrequency}, total_count={TotalCount}, offset={Offset}, data_count={DataCount}";
    }
}
...
internal class RawSms : RawData
{
    ...
    public override int Convert(byte[] value, int offset)
    {
        ...
        offset += 4;
        Text = IsAscii == 1
            ? Encoding.ASCII.GetString(value, offset, Count)
            : Encoding.Unicode.GetString(value, offset, Count);
        offset += Count;
        return offset;
    }
...
internal class RawDfPan : RawData
{
    ...
    public int[] IndexPairs { get; private set; }
...
{
    public int Altitude; // unit: 0.1 m
    public int Heading; // unit: 0.01 deg
    public int Latitude; // unit: 0.000001 deg, positive for N and negative for S
    public int Longitude; // unit: 0.000001 deg, positive for E and negative for W
    public ushort Satellites; // number of satellites;
    public int Speed; // unit: 0.001 m
    public byte Status; // none zero means valid, otherwise invalid
    public uint TimestampNanoseconds; // nanosecond partial of timestamp
    public uint TimestampSeconds; // second partial of timestamp

    public override int Convert(byte[] value, int offset)
    {
        offset = base.Convert(value, offset);
        TimestampSeconds = BitConverter.ToUInt32(value, offset);
        offset += 4;
        TimestampNanoseconds = BitConverter.ToUInt32(value, offset);
        offset += 4;
        Altitude = BitConverter.ToInt32(value, offset);
        offset += 4;
        Latitude = BitConverter.ToInt32(value, offset);
        offset += 4;
        Longitude = BitConverter.ToInt32(value, offset);
        offset += 4;
        Speed = BitConverter.ToInt32(value, offset);
        offset += 4;
        Heading = BitConverter.ToInt32(value, offset);
        offset += 4;
        Satellites = BitConverter.ToUInt16(value, offset);
        offset += 2;
        Status = value[offset];
        offset++;
        return offset;

...
public partial class Eb200
{
    ...
    private object ToSpectrum(RawIfPan data)
    {
        ...
        if (CurFeature == FeatureType.FScne)
            if (StartFrequency.CompareWith(data.Frequency / 1000000d) > 0 ||
                ...
                return null;
        var levels = new short[data.Trace.NumberOfTraceItems];
        for (var i = 0; i < levels.Length; i++) levels[i] = data.DataCollection[i];
    ...
    private object ToFScan(RawFScan data)
    {
        ...
        {
            levels[i] = data.DataCollection[i];
            freqs[i] = data.FreqCollection[i] / 1000000d;
        }
    ...
    private object ToDScan(RawDScan data)
    {
        ...
        {
            levels[i] = data.DataCollection[i];
            freq = data.FreqCollection[i] / 1000000d;
        }

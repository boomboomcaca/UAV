...
// 不访问实例数据，可标记为 static [MR5000A]csharp(CA1822)
...
internal abstract class DataFilterBase : IDataFilter, ICloneable
{
    ...
    public DataFilterBase(IDataSinker sinker)
    {
        Type = GetType();
        Sinker = sinker;
    }
...
internal class RawFilterforDuplexChannel : DataFilterBase
{
    ...
    [NonSerialized] private object _stateLock = new();
    ...
    public override void Receive(object data)
    {
        lock (_stateLock)
        {
            var dfIq = data as RawDfiQforDuplexChannel;
            if (dfIq == null
...
internal class LevelFilterforDuplexChannel : DataFilterBase
{
    ...
    public override void Receive(object data)
    {
        if (Sinker != null && data is RawDfiQforDuplexChannel { GroupOffset: 0, ChannelIndex: 0 or -1 } dfIq)
        {
...
[Serializable]
internal class PhaseDifferenceFilterforDuplexChannel : DataFilterBase
{
    #region 构造函数

    public PhaseDifferenceFilterforDuplexChannel(IDataSinker sinker)
        : base(sinker)
    {
    }
    ...
    public override void Receive(object data)
    {
        lock (_stateLock)
        {
            var dfc = data as RawDfCforDuplexChannel;
            if (dfc == null || dfc.AntennaIndex != _antennaIndex ||
                Math.Abs(dfc.Frequency / 1000000.0d - _frequency) > Epsilon ||
                Math.Abs(dfc.Bandwidth / 1000.0d - _bandwidth) > Epsilon) return;
            if (ValidCount != dfc.SpectraCharacterCount)
            {
                PhaseDiffs = null;
            ...
            ValidCount = dfc.SpectraCharacterCount;
            PhaseDiffs ??= new short[dfc.Characters.Length];
            if (_phaseDiffIntegs == null)
            {
...
[Serializable]
internal class ScanDfPhaseDifferenceFilterforDuplexChannel : DataFilterBase
{
    #region 成员变量
    ...
    public ScanDfPhaseDifferenceFilterforDuplexChannel(IDataSinker sinker)
        : base(sinker)
    {
    }
    ...
    public override void Receive(object data)
    {
        ...
        {
            var dfc = data as RawDfCforDuplexChannel;
            if (dfc == null || dfc.AntennaIndex != _antennaIndex || (dfc.Frequency != _nextFrequency &&
                                                                     Math.Abs(dfc.Frequency / 1000000.0d -
                                                                              _startFrequency) > Epsilon)) return;
            Offset = Utils.GetCurrIndex(dfc.Frequency / 1000000.0d, _startFrequency, _stepFrequency);
            Count = dfc.SpectraCharacterCount;
...
internal class RawFilterforNineChannel : DataFilterBase
{
    ...
    public override void Receive(object data)
    {
        lock (_stateLock)
        {
            var dfIq = data as RawDfiQforNineChannel;
            if (dfIq == null || Math.Abs(dfIq.Frequency / 1000000.0d - _frequency) > Epsilon ||
...
internal class LevelFilterforNineChannel : DataFilterBase
{
    ...
    public override void Receive(object data)
    {
        if (Sinker != null && data is RawDfiQforNineChannel { PacketOffset: 0 } dfIq)
        {
            if (Math.Abs(dfIq.Frequency / 1000000.0d - Frequency) > Epsilon ||
                Math.Abs(dfIq.Bandwidth / 1000.0d - Bandwidth) > Epsilon) return;
            Attenuation = dfIq.Attenuation;
...
internal class PhaseDifferenceFilterforNineChannel : DataFilterBase
{
    ...
    public override void Receive(object data)
    {
        lock (_stateLock)
        {
            var dfc = data as RawDfCforNineChannel;
            if (dfc == null || dfc.AntennaIndex != _antennaIndex ||
                Math.Abs(dfc.Frequency / 1000000.0d - _frequency) > Epsilon ||
                Math.Abs(dfc.Bandwidth / 1000.0d - _bandwidth) > Epsilon) return;
...
{
    event DfRelatedDataArrivedEventHandler DataArrived;
    void Receive(object data);
    void ClearData();
    void StartProcess();
    void StopProcess();
...
internal abstract class DataSinkerBase : IDataSinker
{
    ...
    public event DfRelatedDataArrivedEventHandler DataArrived;
    ...
    {
        if (_dataProcess is not { IsAlive: true })
        {
    ...
    {
        if (_dataProcess is { IsAlive: true })
        {
            ReceivingData = false;
            StopThread = true;
        }
...
internal class DFindRawSinkerforDuplexChannel : DataSinkerBase, IDisposable
{
    ...
    protected override object ToData(object obj)
    {
			...
			_stream.Flush();
#endif
        if (obj is RawFilterforDuplexChannel raw)
        {
 ...
 new int[9, 2] { { 0, 1 }, { 0, 4 }, { 0, 7 }, { 2, 0 }, { 5, 0 }, { 8, 0 }, { 8, 3 }, { 8, 6 }, { 8, 1 } };
            ...
            for (var index = 0; index < raw.GroupCount; ++index)
            {
                ...
                Array.Copy(raw.IqCollection, (raw.ChannelCount * index + 1) * raw.DfSamplingCount * 2,
                    ch2TimeDomainData, 0,
                    length * 2);
...
internal class WbdfRawSinkerforDuplexChannel : DataSinkerBase
{
    ...
    protected override object ToData(object obj)
    {
        ...
        for (var index = 0; index < wbdf.Azimuths.Length; ++index) wbdf.Azimuths[index] = wbdf.Qualities[index] = -1.0f;
        ...
        {
            if (ch1FrequencyDomainData != null)
...
internal class DFindPhaseDifferenceSinkerforDuplexChannel : DataSinkerBase
{
    #region 构造函数

    public DFindPhaseDifferenceSinkerforDuplexChannel(IDictionary<int, IDFindCalibration> dfindCalibrators = null,
        int dfAntennaRef = 2, AntennaRefFrequencyRange[] exceptionalAntennaRefFrequencyRanges = null)
    {
        _dfindCalibrators = dfindCalibrators;
        _dfAntennaRef = dfAntennaRef;
        _exceptionalAntennaRefFrequencyRanges = exceptionalAntennaRefFrequencyRanges;
    }
    ...
    private readonly AntennaRefFrequencyRange[] _exceptionalAntennaRefFrequencyRanges;
    ...
    public override void Receive(object data)
    {
        if (data is PhaseDifferenceFilterforDuplexChannel) base.Receive(data);
    }

    protected override object ToData(object obj)
    {
        var raw = obj as PhaseDifferenceFilterforDuplexChannel;
...
#endif
        var dfAntennaRef = _dfAntennaRef;
        if (_exceptionalAntennaRefFrequencyRanges is { Length: > 0 })
        {
            var frequencyRange = _exceptionalAntennaRefFrequencyRanges.FirstOrDefault(item =>
                raw.Frequency >= item.StartFrequency && raw.Frequency <= item.StopFrequency);
            if (frequencyRange != null) dfAntennaRef = frequencyRange.AntennaReferenceType;
        }
...
internal class WbdfPhaseDifferenceSinkerforDuplexChannel : DataSinkerBase
{
    #region 构造函数

    public WbdfPhaseDifferenceSinkerforDuplexChannel(IDictionary<int, IDFindCalibration> dfindCalibrators = null,
        int dfAntennaRef = 2, AntennaRefFrequencyRange[] exceptionalAntennaRefFrequencyRanges = null)
    {
    ...
    protected override object ToData(object obj)
    {
        ...
        // {
        //     spectrum.Datas[index] = raw.Spectrum[index] / 10.0f;
        // }
        result.Add(spectrum);
...
internal class ScanDfPhaseDifferenceSinkerforDuplexChannel : DataSinkerBase
{
    #region 构造函数

    public ScanDfPhaseDifferenceSinkerforDuplexChannel(IDictionary<int, IDFindCalibration> dfindCalibrators = null,
        int dfAntennaRef = 2, AntennaRefFrequencyRange[] exceptionalAntennaRefFrequencyRanges = null)
    {
        MaximumBufferedPhaseDiff = 2048 * 100;
        MaximumQueueLength = 150;
    ...
    public override void Receive(object data)
    {
        if (data is ScanDfPhaseDifferenceFilterforDuplexChannel) base.Receive(data);
    }

    protected override object ToData(object obj)
    {
        var raw = obj as ScanDfPhaseDifferenceFilterforDuplexChannel;
        var result = new List<object>();
        ...
        for (var index = 0; index < length; ++index)
        {
            ...
            scanDf.Azimuths[index] = scanDf.Qualities[index] = -1.0f;
        }
        ...
        {
            var frequencyRange = _exceptionalAntennaRefFrequencyRanges.FirstOrDefault(item =>
                raw.StartFrequency >= item.StartFrequency && raw.StartFrequency <= item.StopFrequency);
            if (frequencyRange != null) dfAntennaRef = frequencyRange.AntennaReferenceType;
        }
...
internal class SseRawSinkerforNineChannel : DataSinkerBase, IDisposable
{
        ...
        IDictionary<int, float[]> channelPhaseDiffDic = null)
    {
        ...
        if (channelPhaseDiffDic is { Count: > 0 })
        {
    ...
    [DllImport("sse_doa.dll", EntryPoint = "spec_doa", CallingConvention = CallingConvention.StdCall)]
    private static extern int OutputSSE(long frequency, int angleCount, float[] calibratedValues, int samplingCount,
        int[] iq, float aperture, float[] results);

    [DllImport("sse_for_noncorherent_signal.dll", EntryPoint = "sse_noncorherent_signal",
        CallingConvention = CallingConvention.StdCall)]
    private static extern int OutputSSE(long frequency, float[] calibratedValues, int samplingCount, int[] iq,
        float aperture, float[] results);

    [DllImport("signals_number_estimator.dll", EntryPoint = "signals_number_estimator",
        CallingConvention = CallingConvention.StdCall)]
    private static extern int EstimateCountOfAngles(int samplingCount, int[] iq, float[] calibratedValues, float a,
        int b);

    // [DllImport("libsse-df.dll", EntryPoint = "get_sse_doa_in_nine_channel", CallingConvention = CallingConvention.Cdecl)]
    // private static extern void OutputSSE(float[] result, ref int estimatedSSECount, long frequency, float[] calibratedValues, int samplingCount, int[] iq, float aperture, float estimatedSignalCountCoe, int estimatedSignalCountMethod, bool iqBit16, bool coherent, int estimatedMethondInCoherence);
    ...
    protected override object ToData(object obj)
    {
        ...
        if (calibratedValues.Length != 9) Array.Resize(ref calibratedValues, raw.ChannelCount);
			...
			{
				var frequencyBytes = BitConverter.GetBytes(frequencyByHz);
				var apertureBytes = BitConverter.GetBytes(raw.Aperture);
				var sampleCountBytes = BitConverter.GetBytes(raw.DFSamplingCount);
				var calibratedValuesInBytes = new byte[calibratedValues.Length * sizeof(float)];
				Buffer.BlockCopy(calibratedValues, 0, calibratedValuesInBytes, 0, calibratedValuesInBytes.Length);
				_stream.Write(frequencyBytes, 0, frequencyBytes.Length);
				_stream.Write(apertureBytes, 0, apertureBytes.Length);
				_stream.Write(sampleCountBytes, 0, sampleCountBytes.Length);
				_stream.Write(calibratedValuesInBytes, 0, calibratedValuesInBytes.Length);
				_counter++;
			}
			else if (_counter > 200 && _counter <=400)
			{
				var buffer = new byte[raw.IQCollection.Length * sizeof(int)];
				Buffer.BlockCopy(raw.IQCollection, 0, buffer, 0, buffer.Length);
				_stream.Write(buffer, 0, buffer.Length);
				_counter++;
			}
			...
			try
			{
				var tempCalibratedValues = new float[calibratedValues.Length];
				Buffer.BlockCopy(calibratedValues, 0, tempCalibratedValues, 0, tempCalibratedValues.Length * sizeof(float));
				var tempIQCollection = new int[raw.IQCollection.Length];
				Buffer.BlockCopy(raw.IQCollection, 0, tempIQCollection, 0, tempIQCollection.Length * sizeof(int));
				ConvertIQAndPhaseDiffsToReferencChannel(tempCalibratedValues, tempIQCollection, raw.ChannelCount, out referenceChannel);
				signalCount =
 OutputSSE(frequencyByHz, tempCalibratedValues, raw.DFSamplingCount, tempIQCollection, raw.Aperture, results);
			}
			catch
			{
				return null;
			}
			var angleOffsetIndex =
 (int)((raw.AngleOffset + referenceChannel * 360.0f / raw.ChannelCount) / (360.0f / results.Length));
			if (angleOffsetIndex < 0)
			{
				angleOffsetIndex += results.Length;
			}
			var sse = new SDataSSE
			{
				Frequency = raw.Frequency,
				DFBandwidth = raw.Bandwidth,
        ...
        try
        {
            ...
            {
                var countOfAngles =
                    EstimateCountOfAngles(raw.DfSamplingCount, raw.IqCollection, calibratedValues, 1, 3);
                ...
                if (total >= raw.IntegratedSseTimes)
                {
                    ...
                    if (_countOfAngles > 4) _countOfAngles = 4;
            ...
            OutputSSE(frequencyByHz, _countOfAngles < 0 ? 0 : _countOfAngles, calibratedValues, raw.DfSamplingCount,
                raw.IqCollection, raw.Aperture, results);
        }
        catch
        {
            return null;
        ...
        // 	// {
        // 	// 	var countOfAngles = -1;
        // 	// 	OutputSSE(results, ref countOfAngles, frequencyByHz, calibratedValues, raw.DFSamplingCount, raw.IQCollection, raw.Aperture, 1, 3, false, false, 1);
        // 	// 	if (countOfAngles <= 0)
        // 	// 	{
        // 	// 		return null;
        // 	// 	}
        ...
        // 	// 	_countOfAngles = raw.EstimatedSSECount;
        // 	// }
        // 	// OutputSSE(results, ref _countOfAngles, frequencyByHz, calibratedValues, raw.DFSamplingCount, raw.IQCollection, raw.Aperture, 1, 3, false, false, 1);
        // 	if (raw.EstimatedSSECount == -1)
        // 	{
        // 		_countOfAngles = -1;
        // 	}
        ...
        // 		_countOfAngles = raw.EstimatedSSECount;
        // 	}
        // 	OutputSSE(results, ref _countOfAngles, frequencyByHz, calibratedValues, raw.DFSamplingCount, raw.IQCollection, raw.Aperture, 1, 3, false, true, 1);
        // }
        // catch
        // {
        // 	return null;
        ...
        var sse = new SDataSse
        {
            ...
            // SignalCount = raw.EstimatedSSECount,
    ...
    private void ConvertIqAndPhaseDiffsToReferencChannel(float[] phaseDiffs, int[] iq, int channelCount,
        out int referenceChannel)
    {
        referenceChannel = 0;
        if (phaseDiffs == null || iq == null || channelCount <= 0) return;
        if (phaseDiffs.Length != channelCount) Array.Resize(ref phaseDiffs, channelCount);
        var iqLengthPerchannel = iq.Length / channelCount;
        var maxLevel = -9999.0f;
        for (var channelIndex = 0; channelIndex < channelCount; ++channelIndex)
        {
            var iqDataInOneChannel = new int[iqLengthPerchannel];
            Buffer.BlockCopy(iq, channelIndex * iqLengthPerchannel * sizeof(int), iqDataInOneChannel, 0,
                iqDataInOneChannel.Length * sizeof(int));
            var level = Utilities.GetLevel(Array.ConvertAll(iqDataInOneChannel, item => (float)item));
            if (level > maxLevel)
            {
                maxLevel = level;
                referenceChannel = channelIndex;
            }
        }

        if (referenceChannel == 0) return;
        var tempPhaseDiffs = new float[channelCount];
        var refPhaseDiff = phaseDiffs[referenceChannel];
        for (var channelIndex = referenceChannel; channelIndex > 0; --channelIndex)
            phaseDiffs[channelIndex] = phaseDiffs[channelIndex - 1];
        phaseDiffs[0] = 0;
        for (var channelIndex = 0; channelIndex < channelCount - 1; ++channelIndex)
            tempPhaseDiffs[channelIndex] =
                phaseDiffs[(channelIndex + referenceChannel + 1) % (channelCount - 1)] - refPhaseDiff;
        tempPhaseDiffs[channelCount - 1] = tempPhaseDiffs[channelCount - 2] - tempPhaseDiffs[0];
        phaseDiffs = tempPhaseDiffs;
        var tempIq = new int[iq.Length];
        for (var channelIndex = 0; channelIndex < channelCount; ++channelIndex)
            Buffer.BlockCopy(iq, (channelIndex + referenceChannel) % channelCount * iqLengthPerchannel * sizeof(int),
                tempIq, channelIndex * iqLengthPerchannel * sizeof(int), iqLengthPerchannel * sizeof(int));
        iq = tempIq;
    }
...
internal static class DdfUtilities
{
    ...
    private static readonly double[] _sinArray;
    ...
    public static DdfCorrelation GetDdfCorrelation(float[] values, float[] samples, int angles)
    {
        ...
        for (var angleIndex = 0; angleIndex < angles; ++angleIndex)
        {
            ...
            if (correlationResult[angleIndex] < 0.0f)
                // correlationResult[angleIndex] = -correlationResult[angleIndex];
                correlationResult[angleIndex] = 0.0f;
        ...
        {
            Azimuth = azimuth, Quality = quality, Angles = angles,
    ...
    private static double GetSinValue(float degree)
    {
        var temp = (int)((degree % 360 + 360) % 360 * 10);
        return _sinArray[temp];
    }

    private static double GetCosinValue(float degree)
    {
        var temp = (int)((degree % 360 + 360) % 360 * 10);
        return _cosArray[temp];
    }
    ...
    private static float[] GetPhaseDiffsByOffsetInTheory(double frequency, float aperture, int angles, int angleOffset)
    {
        ...
        var radValue = Math.PI / 180.0d;
        var coe = Math.PI / 150.0d;

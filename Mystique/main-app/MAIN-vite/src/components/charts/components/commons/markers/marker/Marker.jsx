...
const Marker = (props) => {
  const { marker, releaseDrag, onReadyDrag, onStopDrag, onSelect, onMove } =
    props;
  ...
  return (
    <>
      {marker && (
        <>
          ...
          <div
            ...
            style={{ top: `calc(${marker?.position.y}% - 2px)` }}
      ...
      >
        ...
        >
          <div className={styles.symbol}>{marker?.markerIndex + 1}</div>
        </div>
      </div>
      {marker?.selected && isMobile() && (
        <div className={styles.markerCaption}>
          <span>MK{marker?.markerIndex + 1}</span>
          <div className={styles.markerContent}>
            <div className={styles.freq}>{marker?.freqInfo?.no}</div>
            <span>{marker?.freqInfo?.unit}</span>
            <div>{timestamp2String(marker?.dataInfo?.timestamp)}</div>
            <div>{marker?.dataInfo?.levelCaption}</div>

...
const workerThread = () => {
    ...
    while (dataStart < count) {
      ...
      if (this.signalIndexes.includes(sineStart)) {
        ...
        for (let i = dataStart; i < dataStart + needLen; i++) {
          datas[i] = ampOffsetMin + Math.random() * ampOffsetRange;
        }
      }
  ...
  function scanMocker(segments, frameInterval, onData) {
    ...
    let prevOver = true;
    const splitInterval = Math.ceil(frameInterval / this.totalSplitTimes);
 ...
 * @param {Function} onData
 */
export const demoData = (options, onData) => {
  // 初始化线程
  const workerJs = `(${workerThread.toString()})()`;
  workerBlob = new Blob([workerJs], { type: "text/javascript" });
  const blogUrl = window.URL.createObjectURL(workerBlob);
  worker = new Worker(blogUrl, {
  ...
  worker.postMessage({ action: "init", data: options });
};

export const getFrame = () => {
  worker.postMessage({ action: "getFrame", data: null });
};

export const disposeMocker = () => {
  if (worker) {

...
function useArea(areacode) {
  ...
  const getAreas = async (params = { rows: 2000, level: 0 }) => {
    const ret = await getList('/dic/areacode', params);
    if (ret.result) {
      if (params.level > 0) {
  ...
  const getSomeAreas = async (code) => {
    ...
    gets.push(getList('/dic/areacode', { rows: 2000, level: 0 }));
    if (!isSpecial) {
      gets.push(getList('/dic/areacode', { rows: 2000, level: 1, 'code.hlk': code.substring(0, 1 * 2) }));
    }
    gets.push(
      getList('/dic/areacode', { rows: 2000, level: 2, 'code.hlk': code.substring(0, 2 * (isSpecial ? 1 : 2)) })
    );

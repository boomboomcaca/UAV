 ...
 * @LastEditTime: 2022-11-23 11:50:12
 */
...
import styles from './PathAxis.module.less';

const PathAxis = (props) => {
  const { value, onValueChange, content, onMouseEnter, datas, mode } = props;
  ...
  const moveLength = useMemo(() => {
    ...
    if (rootConRef.current && axisRef.current) {
      ...
      if (currentIndex < judgeLength - 1) {
        length = currentIndex * rootConWidth;
      } else {
        // 计算有误
        length = axisWidth - rootConWidth;
      }
    }
    return length;
  ...
  const onHover = (a, e, index) => {
    if (datas.length / 2 >= index + 1) {
      setDistance(a.pageX - rootRef.current.offsetLeft - 120);
    } else {
      setDistance(a.pageX - rootRef.current.offsetLeft - 400);
    }

    e.state !== -1 && setShowBubble(true);
    onMouseEnter(e);
  };
  return (
    <div ref={rootRef} className={styles.root}>
      ...
      <div
        ...
        className={classnames(
          styles.bubble,
          mode === 'bottom' ? styles.bottom : null,
          // datas.length / 2 > index + 1 ? styles.left : datas.length / 2 === index + 1 ? styles.center : styles.right,
          showBubble ? styles.showBubble : null,
        )}
      >
        {content}
      </div>

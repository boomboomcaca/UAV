...
const FrequencyInput = (props) => {
  const { value, onValueChange, className, style, decimals, miniValue, maxValue, disable, hideKeys, unavailableKeys } =
    props;
  ...
  return (
    <>
      <div className={classnames(styles.freqInput, className, disable ? styles.disable : '')} style={style}>
        <div className={styles.text} onClick={ssshow}>{`${showValue}`}</div>
      </div>
      <PopUp
        visible={showModal}
        ...
        usePortal
      >
        <div className={styles.freqModal}>
          <Input
            value={text}
            suffix={suffix}
            size="large"

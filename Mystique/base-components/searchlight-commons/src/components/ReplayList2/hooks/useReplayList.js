...
function useReplayList(type, config, request, setSyncData, onSelectChange, showReplay = false) {
  ...
  const getReplayList = (pinfo) => {
    // TODO be careful
    const prepp = {};
    if ((showReplay && !['single', 'segment', 'segments'].includes(type)) || !config.appid) {
      prepp.feature = type;
    } else {
      prepp.frontFuncId = config.appid;
    }
    const param = {
      ...prepp,
      ...pinfo,
  ...
  const onPlaysync = (item /* , setSync */) => {
    // gotoSync(item.id, config.wsTaskUrl, (bo) => {
    //   if (bo) {
  ...
  const onDownload = (tag, dat, condition) => {
    // gotoDownload(dat.id, tag, config.wsTaskUrl, (res) => {
    //   window.console.log(res);
    //   if (res.error) {
    ...
    //     // });
    //     if (window.App && res.result) {
    //       window.App.downloadAndSaveFile(`${config.apiBaseUrl}${res.result}`, 'replayData').catch((re) =>
    ...
    // });
    request({
      url: `log/logFileInfo/getDownloadPath`,
      method: 'post',
      data: { id: dat.id, dataType: tag, ...condition },
    })
      .then((res) => {
        window.console.log(res);
        if (res.result) {
          if (window.App) {
            ...
            message.error({
              key: 'commons-main',
              content: `window.App.downloadAndSaveFile 不存在，无法下载 ${config.apiBaseUrl}${res.result}`,
              duration: 5,
            });
          }
        }
        if (res.error) {
          message.error({ key: 'commons-main', content: res.error.message });
        }
      })
      .catch((rej) => {
        window.console.log(rej);
